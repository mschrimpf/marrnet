...
def render_file(ifilename, ofile_path, ofile_prefix, ofile_ext, views=[[240, 0, 0], [150, -150, 75], [0, 240, 50]],
                resolution=[512, 512], pooling_step=1, method='iso', color=(0.75, 0.75, 0.75), iname='voxels',
                overwrite=True):
...
def render_mat(mat, ofile_path, ofile_prefix, ofile_ext, views=[[240, 0, 0], [150, -150, 75], [0, 240, 50]],
               resolution=[512, 512], pooling_step=1, method='iso', color=(0.75, 0.75, 0.75), overwrite=True):
    for i in range(mat.shape[0]):
        for j, cam_dis in enumerate(views):
            ...
            if not overwrite and os.path.isfile(ofilename):
                ...
                render_voxel(mat[i][0], method=method, filename=ofilename, resolution=resolution,
                             pooling_step=pooling_step, color=color, cam_dis=cam_dis)
                print('==> file ' + ofilename + ' rendered')
    return mat.shape[0]


def render_voxel(mat, filename=None, resolution=[512, 512], pooling_step=1, method='iso', color=(0.75, 0.75, 0.75),
                 cam_dis=(240, 0, 0)):
    ...
    if filename != None:
        ...
        bpy.data.scenes[0].render.filepath = os.path.realpath(filename)
