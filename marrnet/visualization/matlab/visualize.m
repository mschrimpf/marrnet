function visualize(inputfile, indices, step, threshold, outputprefix, outputDir)
...
if isstr(inputfile)
    ...
    voxels = inputfile;
    assert(all(size(voxels) == [1, 128, 128, 128]));
end
...
for index = indices
    ...
    if ~visibility
        saveas(fig,[outputDir outputprefix '_' num2str(index) '_front.bmp']);
    end
    fig = voxel_render(voxel, step, threshold, visibility, 'side');
    if ~visibility
        saveas(fig,[outputDir outputprefix '_' num2str(index) '_side.bmp']);
    end
end
